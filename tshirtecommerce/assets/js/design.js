...
var design={
	...
	teams: {},
	fonts: '',
	drawImage: function (ctx, img, imgX, imgY, W2, H2) {
	  ...
	  function resizeImg (img, w, h) {
		...
		resample_hermite(newCanvas, img.width, img.height, w, h);
		var newImg = new Image();
		newImg.src = newCanvas.toDataURL();
		return newImg;
	  }
	  if (W2 && H2) {
		img = resizeImg(img, W2, H2);
	  }
	...
	ajax:{
		...
		getPrice: function(){
			...
			datas.quantity = jQuery('#sales_goal_input').val();
			console.log(datas.quantity);
			jQuery('.box_design').each(function(index){
							if (jQuery(this).hasClass('active')) {
										jQuery(this).attr('product_id',datas.product_id);
							}
			});
			...
			jQuery.ajax({
				...
				if (data != '')
				{
					if (typeof data.sale != 'undefined')
					{
						...
						design.ajax.calculatorPrice(data);
		...
		calculatorProfit: function(qty){
			$_design = jQuery('.box_design.active');
			var cost_price = $_design.find('#cost_price').val();
			var campaign_price = $_design.find('#campaign_price').val();
			var profit_price = campaign_price - cost_price;
			$_design.find('.profit_per').html('$' + design.ajax.formatPrice(profit_price));
			jQuery("#global_profit").html('$' + parseInt(profit_price*qty) + '+');
		},
		calculatorPrice: function(data){
			/* custom */
			$_design = jQuery('.box_design.active');
			var design_title = jQuery('.product-detail-title').html();

			$_design.find('.design-fpd-title').html(design_title);
			...
			var cost_price = data.sale;
			var suggest_price = suggestedPrices[design.ajax.form().product_id] || data.sale * 2;
			var profit_price = suggest_price - cost_price;

			$_design.find('#cost_price').val(cost_price);

			if(jQuery("#launch-progression li").eq(1).hasClass("active-step") && jQuery(".box_design.active #edited_price").val() != "")
			{
			  jQuery(".box_design.active #campaign_price").val(jQuery(".box_design.active #edited_price").val());
		    }
		    else
		    {
		    	$_design.find('#campaign_price').val(suggest_price);
		    }
		...
		reloadThumbnail: function(){
			...
			var back,front,str_front,str_back;
			var front_images = new Array();
			var back_images = new Array();
			if (typeof design.output.front != 'undefined'){
				front = design.output.front.toDataURL();


			}
			if (typeof design.output.back != 'undefined'){
				back = design.output.back.toDataURL();
			}
			$_design = jQuery('.box_design.active');
			$_design.find('#design_image_back').val(encodeURIComponent(back));
			$_design.find('#design_image_front').val(encodeURIComponent(front));
			//front_images.push(encodeURIComponent(front));
			//back_images.push(encodeURIComponent(back));
			str_front = jQuery('.box_design.active #all_design_image_front').val();
			str_back = jQuery('.box_design.active #all_design_image_back').val();
			if(str_front != "")
			{
			front_images = str_front.split(",");
			back_images = str_back.split(",");
			}
			front_images.push(jQuery(".box_design.active .ssp_color li.shirt-color-sample.current").attr("data-value")+"_break_"+encodeURIComponent(front));
			back_images.push(jQuery(".box_design.active .ssp_color li.shirt-color-sample.current").attr("data-value")+"_break_"+encodeURIComponent(back));
			console.log("doannnF-" + front_images.length);
			if(jQuery("#launch-progression li").eq(1).hasClass("active-step"))
			{
			var color_check ="";

				for (var i = 0; i < jQuery(".box_design.active .ssp_color li.shirt-color-sample").length; i++) {
					if(i > 0){color_check += ",";} else {}
					color_check += jQuery(".box_design.active .ssp_color li.shirt-color-sample").eq(i).attr("data-value");
				};
			jQuery(".box_design.active #color_image_check").val(color_check);
		    }
			$_design.find('#all_design_image_back').val(back_images);
			$_design.find('#all_design_image_front').val(front_images);
			if(jQuery('input[name=feature_side]:checked').val()=='front'){
				jQuery('.box_design.active .product_thumbnail').attr('src',front);
				jQuery('.design_view.back').css('background-image', "url('" + back + "')");
				jQuery('.design_view.front').css('background-image', "url('" + front + "')");
			}else{
				jQuery('.box_design.active .product_thumbnail').attr('src',back);
	...
	products:{
		...
		changeColor: function(e, n)
		{
			...
			var color_hex = jQuery(e).data('color');

			if (jQuery('.box_design.active').length > 0) {
				var container = jQuery('.box_design.active');
				var product_id_active =  jQuery('.box_design.active').attr('product_id');
				var index_color_hex = '';

				jQuery.each(colors_design[product_id_active]['color_hex'], function (key, value) {
					if (value == color_hex) {
						index_color_hex = key;
					}
				});

				color_name = colors_design[product_id_active]['color_title'][index_color_hex];
				...
				if(jQuery('.box_design.active').find('.shirt_color_seleted').val()=='' || JSON.parse(jQuery('.box_design.active').find('.shirt_color_seleted').val())==''){
					jQuery('.box_design.active').find('.shirt_color_seleted').val(JSON.stringify(color_selected));
				...
				get_current_change(container,color_hex,color_name);

				show_bg_shirt(container, color_hex);
			}
			design.ajax.getPrice();
		},
		changeColorExtra: function(e, n)
		{
			if (jQuery('.labView.active .design-area').hasClass('zoom'))
				design.tools.zoom();
			jQuery('#product-list-colors span').removeClass('active');
		...
		changeDesignExtra: function(product_id){
		...
		changeDesign: function(e){
			...
			{
				var min_quantity = jQuery('#quantity').attr('value');
					...
					try {							
						if (item.img == true)
						{
							...
							images.onload = function() {
								...
								design.drawImage(context, images, item.left, item.top, item.width, item.height);
								context.restore();
								if(flipFlg == '1')
								{
									context.restore();
